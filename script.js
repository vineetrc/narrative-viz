document.addEventListener('DOMContentLoaded', function() {
    d3.csv('./data/cleaned_nba_playoff_stats.csv').then(function(data) {
        ...
        svg1.append('g')
            ...
            function updateChart(position, team) {
                let filteredData = data;
                if (position !== 'All') {
                    filteredData = filteredData.filter(d => d.Pos === position);
                }
                if (team !== 'All') {
                    filteredData = filteredData.filter(d => d.Tm === team);
                }
            ...
            positionFilter.on('change', function() {
                ...
                updateChart(selectedPosition, selectedTeam);
            });
    
            teamFilter.on('change', function() {
                const selectedTeam = this.value;
                const selectedPosition = positionFilter.property('value');
                updateChart(selectedPosition, selectedTeam);
            });
        });
    });
