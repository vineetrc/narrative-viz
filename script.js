...
document.addEventListener('DOMContentLoaded', function() {
    d3.csv('./data/top_scorers_by_team.csv').then(function(data) {
        ...
        const positions = Array.from(new Set(allData.map(d => d.Pos))).sort();
        const positionSelect = d3.select('#positionSelect');
        positions.forEach(pos => {
            positionSelect.append('option').attr('value', pos).text(pos);
        });
    ...
    const annotations2 = [
        {
            ...
            x: xScale2(data.find(d => d.Player === "Anthony Davis").Player),
        ...
        },
        {
            ...
            x: xScale2(data.find(d => d.Player === "Damian Lillard").Player),
        ...
        },
        {
            ...
            x: xScale2(data.find(d => d.Player === "Joel Embiid").Player),
...
function filterDataByPosition() {
    const selectedPosition = d3.select('#positionSelect').property('value');
    const filteredData = selectedPosition === "All" ? allData : allData.filter(d => d.Pos === selectedPosition);
    
    d3.select('#chart2 svg').remove(); // Clear the previous chart
    drawScene2(filteredData); // Draw the chart with filtered data
}
